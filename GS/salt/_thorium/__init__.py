...
class ThorState(salt.state.HighState):
    ...
    def __init__(
        ...
        log.info(">>>>>>>>>>> ADRIAN: THORIUM CLASS!!!")
        log.info(">>>>>>>>>>> ADRIAN: self.returners {}".format(str(self.returners)))
        log.info(">>>>>>>>>>> ADRIAN: self.reg_ret {}".format(str(self.reg_ret)))
        regdata = {}
        if self.reg_ret is not None:
            try:
                log.info(">>>>>>>>>>> ADRIAN: THORIUM CLASS RETURNER.load_reg {}".format(str(self.reg_ret)))
    ...
    def call_runtime(self):
        ...
        Execute the runtime
        """
        log.info(">>>>>>>>>>> ADRIAN: THORIUM CLASS RUNTIME")
        ...
        r_start = time.time()
        while True:
            log.info(">>>>>>>>>>> ADRIAN: THORIUM CLASS WHILE LOOP")
            log.info(">>>>>>>>>>> ADRIAN: interval {} recompile {} r_start {}".format(interval, recompile, r_start))
            events = self.get_events()
            log.info(">>>>>>>>>>> ADRIAN: events {}".format(str(events)))
            if not events:
                log.info(">>>>>>>>>>> ADRIAN: NO EVENTS, SLEEPING FOR INTERVAL")
                time.sleep(interval)
                continue
            log.info(">>>>>>>>>>> ADRIAN: EVENTS TO PROCESS, WORKING")
            start = time.time()
            ...
            log.info(">>>>>>>>>>> ADRIAN: start {} r_start {} recompile {}, result: {}".format(start, r_start, recompile, str(start - r_start)))
            if (start - r_start) > recompile:
                log.info(">>>>>>>>>>> ADRIAN: THORIUM start - r_start > recompile")
                cache = self.gather_cache()
                chunks = self.get_chunks()
                if self.reg_ret is not None:
                    log.info(">>>>>>>>>>> ADRIAN: THORIUM CLASS RETURNER.save_reg {}".format(str(self.reg_ret)))
