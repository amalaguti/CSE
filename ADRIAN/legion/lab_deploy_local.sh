...
salt-call pkg.install salt-master
salt-call service.disable salt-master
salt-call service.stop salt-master
salt-call pkg.install python3-pip
salt-call git.clone /opt/legion/ https://github.com/saltstack/legion
salt-call cmd.run "pip3 install -e /opt/legion/."
...
salt-call file.write /opt/legion/legions/legions_start.sh "#!/bin/bash"
salt-call file.set_mode /opt/legion/legions/legions_start.sh 0754
for DEVICE in ${_ENGINE_DEVICES[@]}; do
  echo "Adding entry for $DEVICE on legion start script"
  salt-call file.append /opt/legion/legions/legions_start.sh \
...
salt-call cp.get_file salt://rbac_lab/stop_legion.sh /opt/legion/legions/legions_stop.sh
salt-call file.set_mode /opt/legion/legions/legions_stop.sh 0754


# Create and enable legion service
echo "6- Create and enable legion service"
salt-call cp.get_file salt://rbac_lab/legions.service /etc/systemd/system/legions.service
salt-call service.systemctl_reload
salt-call service.enable "legions.service"
