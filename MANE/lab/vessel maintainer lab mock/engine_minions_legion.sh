...
declare -a eng1_devices=("MOP_A" "MOP_B" "PVU")
...
for engine_serial in ${engines_serials[@]}; do
   ...
   #/usr/local/bin/legion -m1 -l0 --master=127.0.0.1 --rand-uuid --name=$base_minion_name-master_minion &
   #sleep 10
   ...
   for device in ${!devices}; do
     ...
     /usr/local/bin/legion -m1 -l0 --master=10.2.0.10 --rand-uuid --name=$base_minion_name-$device &
     sleep 10
   done
   (( count++ ))
done
